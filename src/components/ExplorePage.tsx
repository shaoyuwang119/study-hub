...
function ExplorePage() {
  ...
  const handleSearch = async () => {
    ...
    const res = await fetch(`http://localhost:3000/api/notes/search?${params}`)
    const data = await res.json()
    setResults(data)
    setLoading(false)
  }
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              className="w-full rounded-lg border border-gray-200 bg-zinc-50 px-3 py-2 text-sm focus:border-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-100"
            />
          </div>
